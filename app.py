...
HOLDOUT_META_PATH = Path("data/processed/cleaning_holdout.csv")


@st.cache_data
def load_data():
    if not HOLDOUT_ENGINEERED_PATH.exists() or not HOLDOUT_META_PATH.exists():
        st.error("Holdout files not found. Run the feature pipeline first.")
        return None, None
    ...
    if len(fe) != len(meta):
        st.warning("Engineered and meta holdout lengths differ. Aligning by index.")
...
if fe_df is None or disp_df is None:
    st.stop()
